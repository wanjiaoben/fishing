...
async function customerPageForOrder(request, env, orderId) {
  ...
  const statusBody = `<main class="auth-page"><section class="auth-shell confirmation-card">${shellTop}<span class="status-pill">${escapeHtml(status || "STATUS UPDATED")}</span><h1>Booking status</h1><p>${escapeHtml(terminalMessage)}</p><p>Activity: ${escapeHtml(row.activity)} · ${escapeHtml(row.activity_date)}</p><p class="booking-ref">Booking reference ${escapeHtml(shortCode)} · WhatsApp +81 70-8952-3968 · info@nice.okinawa</p><p class="safe-close">This page is safe to close.</p></section><footer class="footer">© CATALINA JAPAN K.K. · Okinawa Private Tour</footer></main>`;
